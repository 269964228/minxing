<template>
  <div id="all">
    ...
    <figure class="title">
        ...
        <div>
            <div v-for="(v,i) in attachlist.bwyjlist" :key="i" @click="route(v)" v-loading.fullscreen.lock="loading" class="enclo_div">
                <div>{{v.name}}</div>
            </div>
        </div>      
        <div class="character static_state">附件：</div>
        <div>
            <div v-for="(v,i) in attachlist.common" :key="i" @click="route(v)" class="enclo_div">
                <div>{{v.name}}</div>
            </div>
        </div>
    </figure>
    <figure class="opinion">
        <div class="bgcolor">拟办意见：</div>
        <div class="border">
    ...
    <figure class="nesting">
        <div class="nesting_div">
            <div class="static_state">收文日期：</div>
            <div>{{data.draft.shouwenriqi}}</div>
        </div>
        <div class="nesting_div">
            <div class="static_state">收文总号：</div>
            <div>{{data.draft.shouwenzonghao}}</div>
        </div>
        <div class="nesting_div">
            <div class="static_state">来文文种：</div>
            <div>{{data.draft.laiwenwenzhong}}</div>
        </div>
        <div class="nesting_div">
            <div class="static_state">来文单位：</div>
            <div>{{data.draft.laiwendanwei}}</div>
        </div>
        <div class="nesting_div">
            <div class="static_state">来文字号：</div>
            <div>{{data.draft.laiwenzihao}}</div>
        </div>
        <div class="nesting_div">
            <div class="static_state">份数：</div>
            <div>{{data.draft.fenshu}}</div>
        </div>
        <div class="nesting_div">
        ...
        <div class="nesting_div">
            <div class="static_state">页数：</div>
            <div>{{data.draft.yeshu}}</div>
        </div>
        <div class="nesting_div">
            <div class="static_state">接收人员：</div>
            <div>{{data.draft.jieshourenyuan}}</div>
        </div>
        <div class="nesting_div">
            <div class="static_state">接收部门：</div>
            <div>{{data.draft.jieshoubumen}}</div>
        </div>
        <div class="nesting_div">
            <div class="static_state">办理时限：</div>
            <div>{{data.draft.banlishixian}}</div>
        </div>
        <div class="nesting_div">
        ...
        <div class="nesting_div">
            <div class="static_state">协办部门：</div>
            <div>{{data.draft.xiebanbumen}}</div>
        </div>
        <div class="nesting_div">
            <div class="static_state">办理结果</div>
            <div v-html="data.draft.banlijieguo"></div>
        </div>
    </figure>
    <figure class="opinion">
        <div class="bgcolor">特别提示：</div>
        <div class="border">
            <div>{{data.draft.tebietishi}}</div>
        </div>
    </figure>
    ...
    data() {
        ...
        getSSOTokenFn(json,(json) => {
            ajaxPost(json).then(res=>{
                ...
                // 附件数据处理
                var attachlist = {};
                var attachdata = that.data.attachlist;
                for (var i = 0; i < attachdata.length; i++) {
                    // 判断json对象中是否存在type类型
                    if (attachlist[attachdata[i].type]) {
                        attachlist[attachdata[i].type].push(arra[i]);
                    } else{
                        // 类型不存在，创建一个新的数组
                        var arr = new Array();
                        arr.push(attachdata[i]);
                        attachlist[attachdata[i].type] = arr;
                    }
                }
                that.attachlist = attachlist;
            })
        })
    },methods: {
        route (value) {
